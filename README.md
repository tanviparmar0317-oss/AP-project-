...
|--------|-------------|---------------|
| **Tanvi** | Backend Engineer | Model layer implementation (TCP connection, thread-safe data buffering, digital signal processing algorithms). |
| **Kavy** | Frontend Engineer | View layer implementation (VisPy hardware-accelerated rendering, PySide6 main UI layout, Matplotlib offline inspector, and mock TCP server). |
| **Isha** | Integration & Documentation | ViewModel architecture integration (MVVM data routing, shared constants definition, global stylesheet, and repository documentation). |
...
| **Ch** | Dropdown | Select one of 32 channels for single-channel view |
...
4. Use **Ch** dropdown and **Mode** radio buttons to explore the data.
...
Install all package requirements using:
```bash
pip install -r requirements.txt
```
...
| Package | Purpose |
|---------|---------|
| `numpy` | High-performance array operations, packet reconstruction, and rolling buffer |
| `scipy` | Digital filter design and zero-phase Butterworth bandpass implementation |
| `matplotlib` | Static offline signal plotting |
| `pyside6` | Cross-platform Qt GUI framework, event loop, and UI controls |
| `vispy` | Hardware-accelerated OpenGL real-time signal visualizer |
| `pyopengl` | OpenGL backend interface used by VisPy |
