...
2. Start your TCP server on localhost at your chosen port (e.g. 9000). The server must stream raw float64 data in packets of exactly 4608 bytes (32 channels × 18 samples × 8 bytes each).
...
RMS
A rolling RMS is computed per channel with a window of 50 samples. For sample i: RMS[i] = sqrt(mean(signal[i-49 : i+1]²))

Bandpass Filter
A 4th-order Butterworth bandpass filter is applied using scipy.signal.butter and sosfilt (second-order sections for numerical stability).

Low cutoff: 1.0 Hz
High cutoff: 40.0 Hz
Sample rate: 250 Hz
This passes typical EEG/biosignal frequency content and removes DC drift and high-frequency noise.
...
Architecture follows MVVM: the View knows nothing about TCP or data math, the Model knows nothing about the GUI.# AP-project-
